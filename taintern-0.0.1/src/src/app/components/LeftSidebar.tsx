...
export function LeftSidebar({ isCollapsed, onToggle, onOpenSettings }: SidebarProps) {
  ...
  const renderNavItems = (items: typeof configItems, label: string) => (
    <>
      {!isCollapsed && (
        <div className="px-8 mb-2 mt-4">
          <p className="text-[0.5625rem] font-black uppercase tracking-[0.2em] text-foreground/30">{label}</p>
        </div>
      )}
      <nav className="flex flex-col gap-1 w-full px-4 items-center">
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.to}
              to={item.to}
              className={`relative flex items-center gap-3 rounded-xl px-3 py-2.5 transition-all duration-200 group w-full border-2 ${
                isActive 
            ...
            >
              <item.icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-primary' : 'text-foreground/20 group-hover:text-foreground/30'}`} />
              {!isCollapsed && (
                <div className="min-w-0 flex flex-col">
                  <p className="text-[0.75rem] font-black tracking-tight leading-tight truncate">{item.label}</p>
                  <p className="text-[0.5625rem] text-foreground/40 font-medium mt-0.5 truncate">{item.sub}</p>
                </div>
              )}
            </Link>
          );
  ...
  return (
    <motion.div
      ...
      className="relative h-full shrink-0 flex flex-col z-50 bg-white/5 backdrop-blur-lg border-r-2 border-border/5"
    >
      {/* Logo Section */}
      <div className={`flex items-center gap-3 w-full px-6 py-6 mb-2 ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
        ...
        {!isCollapsed && (
          ...
          >
            <span className="font-black text-xl text-foreground tracking-tighter leading-none font-display">
              PayRoll
            </span>
      ...
      <button
        onClick={onToggle}
        className="absolute -right-4 top-12 z-50 w-8 h-8 rounded-full bg-foreground text-primary shadow-lg flex items-center justify-center hover:bg-foreground/90 transition-all hover:scale-110 active:scale-95 border-2 border-border/20"
      >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pb-4">
        {renderNavItems(configItems, 'Configuration')}
      </div>
